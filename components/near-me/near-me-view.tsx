	...
	function confirmGeoAccess() {
		...
		if (typeof navigator !== "undefined" && navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(
				...
				() => {
					setSuburbError("Location access was denied. Please type your suburb.");
				},
			);
		}
	}
					...
					<div ref={locationRef} className="relative">
						...
						{locationPickerOpen && (
							<div className="absolute left-0 top-full z-40 mt-1.5 w-72 overflow-hidden rounded-xl border border-minuri-silver/70 bg-minuri-white shadow-xl">
								...
								{showGeoConsent ? (
									<div className="border-t border-minuri-silver/30 px-4 py-3 space-y-2">
										<p className="text-xs text-minuri-slate leading-snug">
											Minuri will use your device location to find nearby places. Your coordinates are not stored.
										</p>
										<div className="flex gap-2">
											<button
												type="button"
												onClick={confirmGeoAccess}
												className="flex-1 cursor-pointer rounded-lg bg-minuri-teal px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-minuri-teal/90"
											>
												Allow
											</button>
											<button
												...
												className="flex-1 cursor-pointer rounded-lg border border-minuri-silver/80 px-3 py-1.5 text-xs font-medium text-minuri-slate transition hover:bg-minuri-fog"
											>
												Cancel
											</button>
										</div>
